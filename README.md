...
2. **Качает весь плейлист в локальный кэш** (на диск, по хэшу URL) и играет из
   `file://` → офлайн-устойчивость и плавные переходы. Последний плейлист
   **персистится на диск** — переживает перезапуск даже при недоступном бэкенде.
   Когда рекламы нет (пустая очередь / бэкенд лежит без сохранённого плейлиста) —
   крутятся **дефолтные ролики** (`src/main/defaults.ts`).
...
src/
  main/            main-процесс (Node) — вся логика и сеть
    ...
    defaults.ts     дефолтные ролики (крутятся, когда рекламы нет)
