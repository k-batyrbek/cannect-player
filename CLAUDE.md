...
**Осталось (next steps):**
1. SSH на ключи + выключить парольный вход (сейчас вход по паролю).
2. Камера сама выключается без подключённых камер → системный юнит её
   рестарт-циклит (~30с). Стоит на стороне камеры держать HTTP-сервис живым без
   камер; пока это лечится `Restart=always`.
